...
### Requirement: No valid candidates in interval
The system SHALL detect when a function produces no valid values across the entire interval.

#### Scenario: Function undefined across entire interval
- **WHEN** the solver initializes a population and all candidates evaluate to invalid values (NaN, Infinity, -Infinity)
- **THEN** the solver returns a no-result state with an error message instead of a misleading numerical result

#### Scenario: Nearly all candidates invalid
- **WHEN** fewer than 10% of the population produces valid fitness values after initialization
- **THEN** the solver returns a warning that the function may be undefined in most of the interval
