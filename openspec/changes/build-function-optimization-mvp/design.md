...
### Keep advanced expressions optional

The custom expression path can exist as an advanced mode, but the MVP must work without relying on it. If included, it must use `mathjs` compilation and safe evaluation instead of `eval`.
